...
function statusLabel(game: TodayGame): string {
  ...
  if (game.game_time) {
    return formatUTCTime(game.game_time, {
      hour: 'numeric',
      minute: '2-digit',
      timeZoneName: 'short',
    })
  ...
  <div v-else class="flex flex-col divide-y divide-[var(--p-content-border-color)]">
    <div v-for="game in props.games" :key="game.game_id" class="px-4 py-3">
      ...
      <div class="flex items-center justify-between mb-2">
        ...
        >
          <img src="https://cdn.nba.com/logos/leagues/logo-nba.svg" alt="NBA.com" class="h-3.5" /><i
            class="pi pi-external-link text-[10px]"
          ></i>
        </a>
      </div>
      ...
      <div class="flex gap-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-3 mb-1">
            <img
              :src="game.away_team_logo_url || 'https://cdn.nba.com/logos/nba/fallback.svg'"
              :alt="game.away_team_tricode || 'TBD'"
              class="w-8 h-8 object-contain flex-shrink-0"
              :class="{
                'opacity-40':
                  game.status === 3 && !isWinner(game.away_score, game.home_score, game.status),
                'opacity-30': !game.away_team_logo_url,
              }"
            />
            <div class="flex-1 min-w-0">
              <p
              ...
              >
                <span v-if="game.away_seed" class="text-xs font-normal text-surface-400 mr-1">{{
                  game.away_seed
                }}</span
              ...
              <p v-if="game.away_owner" class="text-xs text-surface-400 truncate">
                ...
                <template v-if="game.away_owner_wins !== null">
                  ...
                  <span
                    v-if="todayRecord(game.away_owner_today_wins, game.away_owner_today_losses)"
            ...
            <span
              v-if="game.status !== 1 && game.away_score !== null"
              class="text-lg font-bold tabular-nums flex-shrink-0"
              :class="{
                'text-surface-400':
                  game.status === 3 && !isWinner(game.away_score, game.home_score, game.status),
              }"
          ...
          <div class="flex items-center gap-3">
            <img
              :src="game.home_team_logo_url || 'https://cdn.nba.com/logos/nba/fallback.svg'"
              :alt="game.home_team_tricode || 'TBD'"
              class="w-8 h-8 object-contain flex-shrink-0"
              :class="{
                'opacity-40':
                  game.status === 3 && !isWinner(game.home_score, game.away_score, game.status),
                'opacity-30': !game.home_team_logo_url,
              }"
            />
            <div class="flex-1 min-w-0">
              <p
              ...
              >
                <span v-if="game.home_seed" class="text-xs font-normal text-surface-400 mr-1">{{
                  game.home_seed
                }}</span
              ...
              <p v-if="game.home_owner" class="text-xs text-surface-400 truncate">
                ...
                <template v-if="game.home_owner_wins !== null">
                  ...
                  <span
                    v-if="todayRecord(game.home_owner_today_wins, game.home_owner_today_losses)"
                    class="text-surface-400"
                  >
                    ·
                    {{
            ...
            <span
              v-if="game.status !== 1 && game.home_score !== null"
              class="text-lg font-bold tabular-nums flex-shrink-0"
              :class="{
                'text-surface-400':
                  game.status === 3 && !isWinner(game.home_score, game.away_score, game.status),
        ...
        <div
          v-if="game.status !== 3 && game.away_win_pct !== null && game.home_win_pct !== null"
          class="flex items-stretch gap-1.5 flex-shrink-0"
        >
          <div class="w-1.5 rounded-full overflow-hidden flex flex-col">
            <div
              class="w-full flex-shrink-0 transition-all duration-500"
              :style="{
                height: fmtPct(game.away_win_pct),
                background: getColors(game.away_team_tricode, game.home_team_tricode).away,
              }"
            ></div>
            <div
              class="w-full flex-1"
              :style="{
                background: getColors(game.away_team_tricode, game.home_team_tricode).home,
              }"
            ></div>
          </div>
          <div class="flex flex-col justify-between py-0.5">
            <span class="text-[10px] font-semibold tabular-nums leading-none text-white">{{
      ...
      <div
        v-if="game.game_label || game.series_game_text || game.series_status_text"
      ...
      >
        <p v-if="game.game_label || game.series_game_text" class="text-xs text-surface-400">
          <template v-if="game.game_label">{{ game.game_label }}</template
          ><template v-if="game.game_label && game.series_game_text"> · </template
          ><template v-if="game.series_game_text"
            >{{ game.series_game_text
            }}<template v-if="game.if_necessary">
              (<span class="hidden sm:inline">if necessary</span
              ><span class="inline sm:hidden">if nec.</span>)</template
            ></template
          >
        </p>
        <div v-else></div>
        <p v-if="game.series_status_text" class="text-xs text-surface-400 flex-shrink-0">
          {{ game.series_status_text }}
        </p>
      </div>
