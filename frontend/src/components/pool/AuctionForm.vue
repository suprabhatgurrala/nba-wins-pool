...
const validations = computed(() => {
  ...
  if (!isEdit.value || canEditConfig.value) {
    if (!isEdit.value) {
      ...
      else if (!/^[0-9]{4}-[0-9]{2}$/.test(form.season))
        errors.season = 'Format YYYY-YY (e.g., 2024-25)'
    }
    ...
    if (
      !Number.isInteger(form.starting_participant_budget) ||
      form.starting_participant_budget <= 0
    ) {
...
function onSubmit() {
  ...
  if (isEdit.value) {
    ...
    if (canEditConfig.value) {
      payload.max_lots_per_participant = Math.max(
        1,
        Math.floor(Number(form.max_lots_per_participant)),
      )
      payload.min_bid_increment = Math.max(1, Math.floor(Number(form.min_bid_increment)))
      payload.starting_participant_budget = Math.max(
        1,
        Math.floor(Number(form.starting_participant_budget)),
      )
    }
...
const showSeasonError = computed(
  () => !isEdit.value && (touched.season || hasSubmitted.value) && !!validations.value.season,
)
const showMaxLotsError = computed(
  () =>
    (!isEdit.value || canEditConfig.value) &&
    (touched.max_lots_per_participant || hasSubmitted.value) &&
    !!validations.value.max_lots_per_participant,
)
const showMinIncError = computed(
  () =>
    (!isEdit.value || canEditConfig.value) &&
    (touched.min_bid_increment || hasSubmitted.value) &&
    !!validations.value.min_bid_increment,
)
const showBudgetError = computed(
  () =>
    (!isEdit.value || canEditConfig.value) &&
    (touched.starting_participant_budget || hasSubmitted.value) &&
    !!validations.value.starting_participant_budget,
)
  ...
  <form @submit.prevent="onSubmit" class="flex flex-col gap-4 min-w-full">
    ...
    <div v-if="!isEdit || canEditConfig" class="flex flex-col gap-2">
      ...
      <div class="flex flex-col gap-2">
        <label for="budget" class="flex w-full justify-between">
          ...
          <Message v-if="showBudgetError" size="small" severity="error" variant="simple">{{
            validations.starting_participant_budget
          }}</Message>
        </label>
        <InputGroup>
      ...
      <div class="flex flex-col gap-2">
        <label for="minInc" class="flex w-full justify-between">
          ...
          <Message v-if="showMinIncError" size="small" severity="error" variant="simple">{{
            validations.min_bid_increment
          }}</Message>
        </label>
        <InputGroup>
          <InputGroupAddon>$</InputGroupAddon>
    ...
    <div v-if="isEdit && !canEditConfig" class="flex flex-col gap-2">
      <p class="text-sm text-surface-400">
        Auction configuration cannot be modified after it has started.
      </p>
    </div>
    ...
    <div v-if="false" class="flex flex-col gap-2">
      ...
      <p class="text-xs text-surface-400">
        Select <b>Start Auction</b> to begin, or <b>Complete Auction</b> when the draft is finished.
      </p>
    </div>
