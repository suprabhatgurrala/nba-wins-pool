...
async def test_update_schedule_job():
    ...
    with patch('nba_wins_pool.job_definitions.get_nba_data_service') as mock_get_service, \
         patch('nba_wins_pool.job_definitions.get_current_season') as mock_season, \
         patch('nba_wins_pool.job_definitions.datetime') as mock_datetime:
        
        mock_service = MagicMock()
        mock_service.update_schedule = AsyncMock()
        mock_get_service.return_value = mock_service
        
        # Mock current time and season
