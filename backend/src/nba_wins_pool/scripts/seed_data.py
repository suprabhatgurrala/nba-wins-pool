        ...
        try:
            ...
            for pool_slug, pool_info in pool_data.items():
                pool_result = await session.execute(
                    select(Pool).where(Pool.slug == pool_slug)
                )
            ...
            for current_pool_slug in pool_slugs:
                ...
                for owner_name in unique_owners:
                    ...
                    member_result = await session.execute(
                        select(Member).where(
                            (Member.name == owner_name) & (Member.pool_id == pool.id)
                        )
                    )
                ...
                for ownership_info in pool_ownerships:
                    ...
                    logger.info(f"    ✅ {ownership_info['owner_name']} -> {team_slug} (${ownership_info['auction_price']})")
                
                # Commit changes for this pool
                await session.commit()
                logger.info(f"  🎉 Successfully seeded {ownerships_created} team ownerships for pool '{current_pool_slug}'!")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Seed NBA Wins Pool database with teams and/or team ownerships"
    )
    ...
    action_group.add_argument(
        "--ownerships", 
        action="store_true", 
        help="Seed team ownerships"
    )
    action_group.add_argument(
        "--pools", 
        action="store_true", 
        help="Seed pools (automatically done with --ownerships)"
    )
    
    # Optional arguments
    parser.add_argument(
        "--pool", 
        help="Specific pool slug to seed team owners for (only used with --ownerships)"
    )
    parser.add_argument(
        "--force", 
        action="store_true", 
